...
const getZoneCommand = function (command, z) {
  const zone = z && typeof z === 'string' ? z.toLowerCase() : '';
  if (['zone2', 'zone3'].includes(zone)) {
    if (command === 'MVL') {
      return zone === 'zone2' ? 'ZVL' : 'VL3';
    }
    return `${zone.toUpperCase()}_${command}`;
  }
  return command;
};
module.exports = {
